...
#
# Ghana-only Tables 1 and 3 for Paper 1. Adapted from 07_make_tables_figures.R
# with all Florida content removed; Table 2 is now produced exclusively by
# 09_make_calibration_selection_figure_table.R (which has access to the
# bootstrap stability metadata that Table 2 needs).
#
# Reads:
...
#
# Table 2 is produced by 09_make_calibration_selection_figure_table.R.
# Table 4 (LCA-derived) is produced by make_paper1_tables_figures.R.
# =============================================================================
...
spec_caption <- sprintf(
  "Canonical specification: SNV_ONLY=%s, DROP_PPE_CAL=%s, DROP_PPE_APPLY=%s, M0_ONLY=%s, SELECTION_METHOD=%s. Tag: %s.",
  SNV_ONLY, DROP_PPE_CAL, DROP_PPE_APPLY, M0_ONLY, SELECTION_METHOD,
  TAG$apply_tag
)
...
# Build the lineage row block: one row per observed lineage level.
# Levels are sorted naturally (L1, L2, L3, L4, ...; "unknown" last).
lineage_levels <- gh_pt %>%
  pull(lineage) %>%
  as.character() %>%
  unique()

# Natural sort: L-prefixed levels by number, then "unknown" / non-L levels last
l_levels  <- sort(grep("^L[0-9]+$", lineage_levels, value = TRUE),
                  decreasing = FALSE)
# Stable order for non-L levels (e.g., "unknown")
nl_levels <- setdiff(lineage_levels, l_levels)
ordered_lineage_levels <- c(l_levels, nl_levels)

lineage_rows <- tibble(
  Characteristic = paste0("Lineage: ", ordered_lineage_levels),
  Ghana          = vapply(ordered_lineage_levels,
                          function(lv) fmt_n_pct(gh_pt$lineage, lv),
                          character(1))
)
...
base_rows <- tibble(
  ...
  Ghana = c(
    ...
    fmt_n_pct(gh_pt$smear, "positive"),
    fmt_n_pct(gh_pt$Cough, "Yes"),
    fmt_n_pct(gh_pt$Fever, "Yes"),
    fmt_n_pct(gh_pt$Hemoptysis, "Yes")
  )
)

depth_row <- tibble(
  Characteristic = "Median sequencing depth, x (median, IQR)",
  Ghana          = fmt_med_iqr(gh_meta$depth_med)
)

table1 <- bind_rows(base_rows, lineage_rows, depth_row)
...
write_table(table1, "table_1_cohort_characteristics",
            paste("Table 1. Ghana cohort characteristics.",
                  ...
                  "sequencing depth at the sample level. Lineage rows are",
                  "one per observed major lineage; sub-lineages collapsed."))
...
table3 <- pp %>%
  ...
  summarise(
    ...
    mean_confirm_sym  = round(mean(confirm_sym [nA > 0 & nB > 0], na.rm = TRUE), 3),
    .groups           = "drop"
  ) %>%
  arrange(pair_type)
...
write_table(table3, "table_3_internal_validity",
            paste("Table 3. Within-visit replicate concordance at primary thresholds, by sample-slot pair type.",
                  ...
                  "mean_confirm_sym = mean of 0.5 * (|A intersect B| / |A| + |A intersect B| / |B|),",
                  "averaging the two asymmetric per-specimen confirmation rates over evaluable pairs."))

message(sprintf("\n[make_paper1_tables_1to3] Done. Tables in %s/", PATHS$tables))
message("Table 2 (calibrated thresholds) is produced by 09_make_calibration_selection_figure_table.R.")
