...
# ============================================================================
# Recompute Looser and Tighter against the stabilized Primary (2026-05-05).
#
# Rationale: 02_calibrate_*.R writes provisional Looser/Tighter using the
# Stage 1 observed-data Primary as the reference. After Stage 2 stabilization,
# the final Primary may differ, and the provisional flanking tiers may no
# longer be axis-wise less/more stringent than the stabilized Primary. The
# Tighter tier in particular can end up looser than Primary on a single axis,
# which breaks the "Looser <= Primary <= Tighter" contract that downstream
# bracketing depends on.
#
# Procedure: from the Stage 1 lexicographic candidate ranking (read from disk),
# require axis-wise dominance against the stabilized Primary:
#   Tighter feasible: DP >= Primary.DP AND AD1 >= Primary.AD1 AND MAF >= Primary.MAF
#                     AND (DP, AD1, MAF) != Primary
#   Looser  feasible: DP <= Primary.DP AND AD1 <= Primary.AD1 AND MAF <= Primary.MAF
#                     AND (DP, AD1, MAF) != Primary
# Pick max-S among Tighter feasible (most stringent) and min-S among Looser
# feasible (least stringent), where S is the rank-sum across the three axes.
# Use top-K = 10 from cal_ranked; widen to top-K = 30 if no feasible candidate
# at K = 10. If no feasible candidate at K = 30 either, the tier is set to
# NULL and downstream code (03_apply_thresholds.R, 09_*.R) handles NULL.
# ============================================================================

TOP_K_PRIMARY  <- 10L
TOP_K_FALLBACK <- 30L

grid_summary_path <- if (SELECTION_METHOD == "pareto") {
...
cal_grid_full <- readr::read_csv(grid_summary_path, show_col_types = FALSE)

cal_ranked_for_ladder <- cal_grid_full %>%
  filter(pass_guards) %>%
  arrange(prop_both0,
          desc(prop_overlap),
          desc(jacc_med_eval),
          ...
          desc(conf_med_eval),
          desc(r2_med_eval)) %>%
  mutate(S = min_rank(DP_min) + min_rank(AD1_min) + min_rank(MAF_min))

primary_DP  <- modal_cell$DP_min
primary_AD1 <- modal_cell$AD1_min
primary_MAF <- modal_cell$MAF_min

cand_k  <- cal_ranked_for_ladder %>%
  slice_head(n = min(TOP_K_PRIMARY,  nrow(cal_ranked_for_ladder)))
cand_k2 <- cal_ranked_for_ladder %>%
  slice_head(n = min(TOP_K_FALLBACK, nrow(cal_ranked_for_ladder)))

not_eq_primary <- function(df) {
  df %>% filter(!(DP_min  == primary_DP  &
                  AD1_min == primary_AD1 &
                  MAF_min == primary_MAF))
}
tighter_feasible <- function(df) {
  df %>% filter(DP_min  >= primary_DP,
                AD1_min >= primary_AD1,
                MAF_min >= primary_MAF) %>% not_eq_primary()
}
looser_feasible <- function(df) {
  df %>% filter(DP_min  <= primary_DP,
                AD1_min <= primary_AD1,
                MAF_min <= primary_MAF) %>% not_eq_primary()
}
pick_max_S <- function(df) {
  df %>% arrange(desc(S), desc(DP_min), desc(AD1_min), desc(MAF_min)) %>%
    slice_head(n = 1)
}
pick_min_S <- function(df) {
  df %>% arrange(S, DP_min, AD1_min, MAF_min) %>%
    slice_head(n = 1)
}

# Tighter
cal_tighter           <- pick_max_S(tighter_feasible(cand_k))
fallback_used_tighter <- FALSE
if (nrow(cal_tighter) == 0) {
  cal_tighter           <- pick_max_S(tighter_feasible(cand_k2))
  fallback_used_tighter <- nrow(cal_tighter) > 0
}
if (nrow(cal_tighter) == 0) {
  cal_tighter <- NULL
  log_msg("\n[02b] NOTE: no Tighter cell axis-wise dominates the stabilized")
  log_msg("           Primary at top-K = %d or %d; Tighter tier is NULL.",
          TOP_K_PRIMARY, TOP_K_FALLBACK)
}

# Looser
cal_looser           <- pick_min_S(looser_feasible(cand_k))
fallback_used_looser <- FALSE
if (nrow(cal_looser) == 0) {
  cal_looser           <- pick_min_S(looser_feasible(cand_k2))
  fallback_used_looser <- nrow(cal_looser) > 0
}
if (nrow(cal_looser) == 0) {
  cal_looser <- NULL
  log_msg("\n[02b] NOTE: no Looser cell is axis-wise dominated by the")
  log_msg("           stabilized Primary at top-K = %d or %d; Looser tier is NULL.",
          TOP_K_PRIMARY, TOP_K_FALLBACK)
}

to_thr_list <- function(row) {
  if (is.null(row) || nrow(row) == 0) return(NULL)
  list(DP_min = row$DP_min, AD1_min = row$AD1_min,
       MAF_min = row$MAF_min, MAF_max = MAF_MAX)
}
thr_looser_new  <- to_thr_list(cal_looser)
thr_tighter_new <- to_thr_list(cal_tighter)

# Overwrite tier files with the dominance-recomputed cells (or remove them
# if NULL, so a stale file from a prior run cannot mislead downstream code)
looser_path  <- file.path(PATHS$calibration, "thr_looser.rds")
tighter_path <- file.path(PATHS$calibration, "thr_tighter.rds")

if (!is.null(thr_looser_new)) {
  saveRDS(thr_looser_new, looser_path)
} else if (file.exists(looser_path)) {
  file.remove(looser_path)
}
if (!is.null(thr_tighter_new)) {
  saveRDS(thr_tighter_new, tighter_path)
} else if (file.exists(tighter_path)) {
  file.remove(tighter_path)
}

# Refresh consolidated thresholds.rds with all three slots
existing_thr$looser  <- thr_looser_new
existing_thr$tighter <- thr_tighter_new
saveRDS(existing_thr, file.path(PATHS$calibration, "thresholds.rds"))

log_msg("\n[02b] Looser/Tighter recomputed against stabilized Primary with axis-wise dominance:")
if (!is.null(thr_looser_new)) {
  log_msg("  Looser:  DP=%g, AD1=%g, MAF=%g  (S = %d%s)",
          thr_looser_new$DP_min, thr_looser_new$AD1_min, thr_looser_new$MAF_min,
          cal_looser$S,
          if (fallback_used_looser)
            sprintf("; required top-K=%d fallback", TOP_K_FALLBACK) else "")
} else {
  log_msg("  Looser:  NULL (no axis-wise feasible candidate)")
}
log_msg("  Primary: DP=%g, AD1=%g, MAF=%g  (stabilized)",
        primary_DP, primary_AD1, primary_MAF)
if (!is.null(thr_tighter_new)) {
  log_msg("  Tighter: DP=%g, AD1=%g, MAF=%g  (S = %d%s)",
          thr_tighter_new$DP_min, thr_tighter_new$AD1_min, thr_tighter_new$MAF_min,
          cal_tighter$S,
          if (fallback_used_tighter)
            sprintf("; required top-K=%d fallback", TOP_K_FALLBACK) else "")
} else {
  log_msg("  Tighter: NULL (no axis-wise feasible candidate)")
}
