...
#            a star; the four named cells annotated by hollow markers
...
# ---- Panel A: (Jaccard, Confirm) plane -------------------------------------
panel_A_subtitle <- sprintf(
  "Eligible: %s (black). Markers: down-triangle = Looser, 
  circle = Primary (red = Stage 2 stabilized, blue = Stage 1 observed), up-triangle = Tighter.",
  eligible_label)
...
# ---- Combine panels --------------------------------------------------------
caption_txt <- paste(
  "Three-tier calibration ladder (all real-grid cells):",
...
if (!modal_eq_observed) {
  ...
  footnote_row <- tibble(
    Selection = sprintf(
      "Note: Stage 1 observed-data primary (%s) was (DP=%g, AD1=%g, MAF=%g), selected in %.1f%% of bootstrap reps; 
...
build_latex_table <- function(t2, modal_eq_obs, thr_obs, prop_obs, method) {
  ...
  if (!modal_eq_obs) {
    out <- c(out,
             sprintf(
               "\\\\\\footnotesize\\textit{Stage 1 observed-data %s primary:} (DP=%g, AD1=%g, MAF=%g), bootstrap selection rate %.1f\\%%; 
...
# ---- Console summary -------------------------------------------------------
message(sprintf("\n[09] Three-tier ladder (method = %s):", SELECTION_METHOD))
if (!is.null(thr_looser))
...
if (!modal_eq_observed) {
  message(sprintf(
    "\nFootnote: Stage 1 primary (%s; DP=%g, AD1=%g, MAF=%g) selected in %.1f%% of reps; 
    overridden by stabilized modal.",
    SELECTION_METHOD,
    thr_observed$DP_min, thr_observed$AD1_min, thr_observed$MAF_min,
    100 * stability$prop_observed_primary))
}
