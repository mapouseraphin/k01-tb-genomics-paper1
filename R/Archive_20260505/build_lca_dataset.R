...
#   maf_bin         - factor: 4 bins
#                       [0.02,0.05), [0.05,0.10), [0.10,0.25), [0.25,MAF_max]
...
#
# Locked design decisions (post 2026-05-03):
#   - Universe: union of (CHROM,POS,REF,ALT) keys with at least one PASS GATK
#       call across the pair. (Sens B, which skipped the FILTER==PASS gate, was
#       retired -- the GATK variant table contains only FILTER==PASS calls by
#       upstream construction, so the gate is operationally vacuous as a
#       sensitivity.)
#   - SNV-only: indels dropped.
#   - PE/PPE: retained in universe, used as stratifier (NOT excluded). PE/PPE
#       exclusion is operated upstream by 03_apply_thresholds.R when
...
#   - MAF binning: single 4-bin scheme. The merged_low Sens C was retired
#       because the M3a fitting set drops [0.02, 0.05) regardless (zero
#       replicate disagreements -> unidentified), making the binning-resolution
#       question moot.
#
# Inputs:
...
log_msg("LCA spec: lca_tag=%s", TAG$lca_tag)
...
# Enforce FILTER == PASS pre-screen. This is defensive: gh_variants.rds is
# typically PASS-only upstream, but we re-enforce here so the universe is
# unambiguous.
n0 <- nrow(gh_v)
if ("FILTER" %in% names(gh_v)) {
  gh_v <- gh_v %>% filter(FILTER == "PASS")
  log_msg("Universe step 1 -- FILTER==PASS enforced: %s -> %s",
          format(n0, big.mark = ","), format(nrow(gh_v), big.mark = ","))
} else {
  log_msg("Universe step 1 -- no FILTER column; assuming all PASS upstream.")
}
...
n_pid_na  <- sum(is.na(lca$patientId))
n_slot_na <- sum(is.na(lca$slot_a) | is.na(lca$slot_b))
if (n_pid_na > 0 || n_slot_na > 0) {
  log_msg("\nWARNING: NA values present after metadata join:")
  log_msg("  NA patientId: %d (expected 0 after upstream pair filter)", n_pid_na)
  log_msg("  NA slot_a or slot_b: %d (parse_slot failure -- investigate)", n_slot_na)
  affected <- lca %>%
    filter(is.na(patientId) | is.na(slot_a) | is.na(slot_b)) %>%
    distinct(sampleA, sampleB, slot_a, slot_b)
  log_msg(paste(capture.output(print(as.data.frame(affected),
                                      row.names = FALSE)),
                collapse = "\n"))
  log_msg("Dropping these rows from LCA dataset.")
  lca <- lca %>% filter(!is.na(patientId), !is.na(slot_a), !is.na(slot_b))
}
