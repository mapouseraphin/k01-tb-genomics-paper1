...
confirm_rate_pair <- function(a_tbl, b_tbl) {
  ...
  c(confirm_both   = mean(a_has & b_has),
    confirm_a_to_b = if (length(a_sites)) mean(a_sites %in% b_sites) else NA_real_,
    confirm_b_to_a = if (length(b_sites)) mean(b_sites %in% a_sites) else NA_real_)
}
