...
log_msg <- function(...) {
  args <- list(...)
  # Single-arg call -> treat as literal string (preserves any % characters in
  # pre-formatted output like print(table)). Multi-arg call -> sprintf format.
...
# Note: as of the 2026-04-30 metadata-rescue patch in 00_prep_metadata.R,
# gh_meta now includes samples whose medianCov was NA but whose call-level
# DP meets the depth floor. The downstream sampleA -> gh_meta join therefore
# succeeds for these samples. The upstream gh_pairs filter that was briefly
# inserted here has been removed; the metadata fix at source is the correct
# location for this concern.
...
# ---- patient/HIV joins -----------------------------------------------------
# After the upstream gh_pairs filter (above), every sample in every pair has
# a record in gh_meta. The simple sampleA-based join is therefore sufficient;
# no rescue cascade is needed. HIV retains the original sampleA->sampleB
# fallback (a sample may have HIV NA in gh_meta even when present).
lca <- lca %>%
