...
#
# Hybrid Pareto calibration. Drop-in alternative to 02_calibrate_lexicographic.R;
# writes the same output filenames (thr_primary.rds, thr_tighter.rds,
# thr_loose.rds, thresholds.rds, cal_lexicographic_thresholds.csv) so all
# downstream consumers work without modification.
#
# RATIONALE (2026-04-30):
#   The lexicographic-only selection was found to be unstable on the AD1
#   axis under nonparametric pair-level bootstrap (B=1000): canonical
#   AD1=1 selected in 48.5% of reps, AD1=6 in 50.4%, joint canonical cell
#   in 19.2% of reps. The lex hierarchy artificially favors low AD1 (more
#   permissive -> more evaluable pairs -> higher Jaccard via inclusion).
#   Hybrid Pareto avoids this bias by selecting the cell closest to the
#   (1, 1) ideal point on the (Jaccard, Confirm) plane, which does not
#   reward permissiveness as such.
#
# THREE-LEVEL EVIDENTIARY LADDER (unchanged from lex):
#   loose   : no calibration filter (sentinel DP=0, AD1=0, MAF=0)
#   primary : Pareto frontier point closest to (1, 1) on (Jaccard, Confirm)
#             tiebreak: highest r2_med_eval, then highest stringency
#   tighter : frontier point with max stringency among non-primary
#             (fallback: top-K guard-passing if frontier has only 1 point)
#
# PARETO METHOD:
#   Objectives are the QUALITY METRICS (jacc_med_eval, conf_med_eval). The
#   (DP, AD1, MAF) grid is the search space; each grid cell is one candidate
#   point on the (J, C) plane. Cell A dominates cell B iff A_J >= B_J and
#   A_C >= B_C with at least one strict; the Pareto frontier is the set of
#   non-dominated cells.
#
...
# Cell i is dominated iff there exists j != i such that
#   J[j] >= J[i] AND C[j] >= C[i], with at least one strict inequality.
# Cells with NA on either axis cannot be evaluated; they are excluded from
# the frontier (and from the dominated set against them).
...
# dist_to_ideal = euclidean distance to (1, 1) on (J, C) plane.
# Tiebreak: higher r2_med_eval (per-call MAF concordance), then higher
# stringency rank (DP_min, AD1_min, MAF_min).
...
# ---- Tighter via stringency on the frontier --------------------------------
...
if (nrow(frontier) >= 2) {
  ...
  # Fallback: frontier has only 1 point. Use top-K guard-passing by
  # lex-style ranking as the candidate pool for tighter.
  ...
  cand_pool  <- cal_ranked %>%
    ...
    add_stringency()
  pool_label <- sprintf("Top-%d guard-passing (frontier sparse; %d frontier point)",
                        min(TOP_K_FALLBACK, nrow(cal_ranked)), nrow(frontier))
}

message(sprintf("Tighter candidate pool: %s.", pool_label))
...
if (nrow(cal_tighter_hp) == 0) {
  cal_tighter_hp <- NULL
  message("\nNOTE: no tighter candidate available; pool exhausted.")
}

# ---- Loose sentinel --------------------------------------------------------
cal_loose_hp <- LOOSE_SENTINEL %>%
  mutate(
    prop_both0       = NA_real_,
...
selected <- bind_rows(
  cal_loose_hp   %>% mutate(selection = "loose")   %>% select(any_of(common_cols)),
...
# selection method is recorded in the file's metadata via the script that
# produced it (cal_hybrid_pareto_frontier.csv exists alongside).
...
# Also save as observed-data backup so the bootstrap script (02b) can
# compare the bootstrap-stabilized primary against the observed-data
# primary, and so we can restore if needed.
...
message("\n[02_calibrate_hybrid_pareto] Done. Thresholds written to ",
        PATHS$calibration, "/")
message(sprintf("  Primary: DP=%g, AD1=%g, MAF=%g (closest-to-ideal on Pareto frontier)",
