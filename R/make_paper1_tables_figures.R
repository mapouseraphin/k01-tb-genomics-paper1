...
#
# v3 (2026-05-03): M3b retired; M3a is unconditional primary. Supp Fig S3
# / Supp Table S3 (formerly M3a-vs-M3b) are now produced by
# cross_spec_sens_a_comparison.R as canonical-vs-Sens-A. Hierarchical
# prior sensitivity sweep relabeled: primary = HN(0, 2.0), mid = HN(0, 1.0),
# tight = HN(0, 0.5).
#
...
#
# Supplemental:
#   Supp Fig S1. PPC observed vs predicted (M3a primary, ~24 cells after
#                [0.02, 0.05) MAF bin drop).
#   Supp Fig S2. Prior sensitivity for M2 (sigma_u half-normal scales
#                0.5 / 1.0 / 2.0; primary is 2.0 post 2026-05-03).
#   Supp Fig S3. (RETIRED here; now produced by cross_spec_sens_a_comparison.R
#                as canonical-vs-Sens-A.)
...
#   Supp Table S3. (RETIRED here; now produced by cross_spec_sens_a_comparison.R.)
#
...
# Post 2026-05-03: PRIOR_SCALES renamed; primary=2.0, mid=1.0, tight=0.5.
# fit_hierarchical_loose_summary.csv no longer exists.
...
summ_m2_pri <- read_csv(file.path(FITS_PATH, "fit_hierarchical_primary_summary.csv"),
                        show_col_types = FALSE)  # HN(0, 2.0), Paper 1 primary
summ_m2_tig <- read_csv(file.path(FITS_PATH, "fit_hierarchical_tight_summary.csv"),
                        show_col_types = FALSE)  # HN(0, 0.5), sensitivity
summ_m2_mid <- read_csv(file.path(FITS_PATH, "fit_hierarchical_mid_summary.csv"),
                        show_col_types = FALSE)  # HN(0, 1.0), sensitivity
summ_m2_gc  <- read_csv(file.path(FITS_PATH, "fit_hierarchical_globalclass_summary.csv"),
                        show_col_types = FALSE)
# summ_m3b: RETIRED 2026-05-03. M3b convergence-failed on the recovered cohort
# (R-hat 1.024, 506 divergent transitions). M3a is the unconditional primary.
# Cross-spec (canonical vs Sens A) sensitivity comparison moved to
...
prior_data <- bind_rows(
  ...
  summ_m2_mid %>% mutate(prior = "half-normal(0, 1.0)"),
  summ_m2_pri %>% mutate(prior = "half-normal(0, 2.0)")  # primary post 2026-05-03
) %>%
...
supp_fig2 <- ggplot(prior_data,
  ...
  labs(x = "Posterior median (95% CrI)",
       y = "Parameter",
       caption = paste("Prior sensitivity for M2.",
                       "Primary post 2026-05-03 is half-normal(0, 2.0);",
                       "the two narrower priors are the sensitivity sweep.")) +
  theme_paper1() +
...
# =============================================================================
# Supplemental Fig S3 (RETIRED HERE)
# =============================================================================
# Formerly: M3a vs M3b comparison.
# Now produced by cross_spec_sens_a_comparison.R as canonical (PE/PPE retained)
# vs Sens A (PE/PPE excluded). Both runs must complete before that script
# emits supp_fig_S3_canonical_vs_sensA.pdf.
message("Supp Fig S3: produced separately by cross_spec_sens_a_comparison.R.")
...
# Supp Table S3: RETIRED HERE.
# Now produced by cross_spec_sens_a_comparison.R as
# supp_table_S3_canonical_vs_sensA.csv. Both canonical and Sens A runs must
# complete before that file is written.
